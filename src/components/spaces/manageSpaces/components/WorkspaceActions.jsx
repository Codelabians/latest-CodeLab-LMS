...
const WorkspaceActions = ({ title }) => {
  ...
  const isCount =
    categories?.data?.total_types == 0 ? 0 : categories?.data?.total_types;
  ...
  const [updateItem, { isLoading: isUpdating }] = usePostMutation();
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingItem) {
        const path =
          title === "Categories"
            ? `/admin/workspace-categories/update-wc/${editingItem.uuid}?_method=PATCH`
            : `/admin/workspace-types/update-wt/${editingItem.uuid}?_method=PATCH`;
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const path =
        title === "Categories"
          ? `/admin/workspace-categories/delete-wc/${itemToDelete.uuid}`
              ...
              {showAddForm && (
                <div className="mb-6 p-4 bg-gray-50 rounded-lg">
                  <h4 className="text-lg font-semibold mb-4 text-gray-800">
                    ...
                    {title === "Categories" ? "Category" : "Type"}
                  </h4>
                  <FormInput
                    label="Name"
                    name="name"
