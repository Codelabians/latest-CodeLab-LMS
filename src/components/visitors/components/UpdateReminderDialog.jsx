...
const UpdateReminderDialog = ({ open, visitor, onCancel, onConfirm, isLoading }) => {
  ...
  useEffect(() => {
    if (!open) return;
    setServerError("");
    setDate(visitor?.reminder_date || "");
    setNote(visitor?.reminder_note || "");
  }, [open, visitor]);
  ...
  const handleSave = async () => {
    ...
    if (!date && !note.trim()) {
      // Clear both — backend allows null payload to clear stale dates.
    ...
    >
      <div className="w-full max-w-md p-6 bg-white shadow-2xl rounded-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <h3 className="text-base font-semibold text-center" style={{ color: TEXT_PRIMARY }}>
          Reminder for &ldquo;{visitor.name}&rdquo;
        </h3>
        <p className="mt-1.5 text-sm text-center" style={{ color: TEXT_SECONDARY }}>
          Leave both fields blank to clear an existing reminder.
        </p>
