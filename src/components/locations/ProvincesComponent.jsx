...
import {
  Plus, Search, Pencil, Trash2, Map, X,
...
/* ───────────────── add/edit modal ───────────────── */
const ProvinceFormModal = ({ open, mode, initial, onCancel, onSubmit, isLoading }) => {
  ...
  if (!open) return null;
  const nameErr = !name.trim() ? "Name is required" : name.length > 255 ? "Max 255 chars" : "";
  const codeErr = code && code.length > 8 ? "Max 8 chars" : "";
  const visibleErr = touched ? (nameErr || codeErr) : "";

  const handleSave = async () => {
    setTouched(true);
    if (nameErr || codeErr) return;
    setServerError("");
    const res = await onSubmit({ name: name.trim(), code: code.trim() || null, is_active: isActive });
      ...
      <div className="w-full max-w-md overflow-hidden bg-white shadow-2xl rounded-2xl" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-[15px] font-bold" style={{ color: TEXT_PRIMARY }}>
                {mode === "edit" ? "Edit Province" : "Add Province"}
              </h3>
              <p className="text-[11.5px] mt-0.5" style={{ color: TEXT_MUTED }}>
                Administrative unit — the dropdown for cities groups by these.
              </p>
            </div>
          </div>
        ...
        <div className="px-5 py-5">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div>
              <label className="block mb-2" style={{ fontSize: 12, fontWeight: 600, color: TEXT_PRIMARY }}>Code (optional)</label>
              <input
                type="text" value={code} onChange={(e) => setCode(e.target.value.toUpperCase())} maxLength={8}
                placeholder="PB"
                style={{ background: SURFACE_HOVER, border: `1px solid ${codeErr ? "#FCA5A5" : BORDER}`, color: TEXT_PRIMARY, width: "100%", height: 40, padding: "0 12px", borderRadius: 8, fontSize: 13, outline: "none", fontFamily: "'Montserrat', sans-serif", textTransform: "uppercase" }}
              />
              {codeErr && <p className="mt-1.5 text-[11.5px] font-medium" style={{ color: BRAND_RED }}>{codeErr}</p>}
            </div>
            <label className="flex items-center gap-3 p-3 cursor-pointer rounded-lg"
              ...
              <div className="flex-1">
                ...
                <div className="text-[11px]" style={{ color: TEXT_MUTED, marginTop: 1 }}>Inactive provinces are hidden from dropdowns.</div>
              </div>
            </label>
          </div>
        </div>
        ...
        <div className="flex items-center justify-end gap-2 px-5 py-3" style={{ borderTop: `1px solid ${BORDER}`, background: "#FAFBFC" }}>
          ...
          >
            {isLoading ? (<><Loader2 size={14} className="mr-1.5 animate-spin" />Saving…</>) : mode === "edit" ? "Save changes" : "Create province"}
          </button>
        </div>
      </div>
    </div>
...
/* ───────────────── delete dialog ───────────────── */
const DeleteDialog = ({ open, province, onCancel, onConfirm, isLoading }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4"
      ...
      <div className="w-full max-w-md p-6 bg-white shadow-2xl rounded-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <h3 className="text-base font-semibold text-center" style={{ color: TEXT_PRIMARY }}>
          Delete &ldquo;{province.name}&rdquo;?
        </h3>
        <p className="mt-2 text-sm text-center" style={{ color: TEXT_SECONDARY }}>
          ...
          <span className="font-semibold" style={{ color: TEXT_PRIMARY }}>
            {province.cities_count ?? 0} {province.cities_count === 1 ? "city" : "cities"}
          </span>{" "}
          attached to it. Cannot be undone.
        </p>
...
const ProvincesComponent = () => {
  ...
  const canManage = hasPermission(user, "manage provinces");

  const [page, setPage] = useState(1);
  const [perPage, setPerPage] = useState(10);
  const [search, setSearch] = useState("");
  const [debouncedSearch, setDebouncedSearch] = useState("");
  const [formModal, setFormModal] = useState({ open: false, mode: null, province: null });
  const [deleteDialog, setDeleteDialog] = useState({ open: false, province: null });
  ...
  const { data, error, isLoading, isFetching } = useGetQuery(
    { path: "/core/provinces", params: queryParams },
  ...
  const openAdd = () => setFormModal({ open: true, mode: "add", province: null });
  const openEdit = (p) => setFormModal({ open: true, mode: "edit", province: p });
  const closeForm = () => setFormModal({ open: false, mode: null, province: null });
  const openDelete = (p) => setDeleteDialog({ open: true, province: p });
  const closeDelete = () => setDeleteDialog({ open: false, province: null });

  const handleSubmit = async (payload) => {
    try {
      if (formModal.mode === "edit") {
        await updateP({ path: `/core/provinces/${formModal.province.uuid}`, body: payload }).unwrap();
        showToast("Province updated", "success");
      } else {
        await createP({ path: "/core/provinces", body: payload }).unwrap();
        showToast("Province created", "success");
      }
      closeForm();
      return { error: null };
    } catch (err) {
      ...
      return { error: firstFieldError || err?.data?.message || "Could not save province." };
    }
  };

  const handleDelete = async () => {
    try {
      await deleteP({ path: `/core/provinces/${deleteDialog.province.uuid}` }).unwrap();
      showToast("Province deleted", "success");
      closeDelete();
    } catch (err) {
      showToast(err?.data?.message || "Failed to delete province.", "error");
    }
  };
    ...
    >
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-[18px] font-bold" style={{ color: TEXT_PRIMARY, letterSpacing: "-0.01em" }}>Provinces</h1>
            <p className="text-[12px] mt-0.5" style={{ color: TEXT_MUTED }}>Administrative units — cities belong to provinces.</p>
          </div>
        </div>
        {canManage && (
          ...
          >
            <Plus size={15} strokeWidth={2.25} /> Add Province
          </button>
        )}
      </div>
        ...
        <table className="w-full">
          <thead style={{ background: "#F8FAFC", borderBottom: `1px solid ${BORDER}` }}>
            <tr>
              ...
              <th className="px-5 py-3 text-left"><span style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.04em", textTransform: "uppercase", color: TEXT_SECONDARY }}>Code</span></th>
              <th className="px-5 py-3 text-left"><span style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.04em", textTransform: "uppercase", color: TEXT_SECONDARY }}>Cities</span></th>
          ...
          <tbody>
            ...
            {!isLoading && error && (
              <tr><td colSpan={6} className="px-5 py-10 text-center">
                <div className="inline-flex items-center gap-2 px-3 py-2 rounded-lg" style={{ background: BRAND_RED_TINT, color: BRAND_RED }}>
                  <AlertTriangle size={14} /><span className="text-sm font-semibold">Couldn't load provinces.</span>
                </div>
              </td></tr>
            )}
            {!isLoading && !error && rows.length === 0 && (
              <tr><td colSpan={6} className="px-5 py-12 text-center text-[13px]" style={{ color: TEXT_MUTED }}>
                No provinces yet.
              </td></tr>
            )}
            {!isLoading && !error && rows.map((p, i) => (
              ...
              >
                ...
                <td className="px-5 py-3 text-sm" style={{ color: TEXT_PRIMARY, fontWeight: 600 }}>{p.name}</td>
                <td className="px-5 py-3 text-sm">
                  {p.code ? (<code className="px-2 py-0.5 rounded text-[11.5px]" style={{ background: "#F1F5F9", color: TEXT_SECONDARY }}>{p.code}</code>) : <span style={{ color: TEXT_MUTED }}>—</span>}
                </td>
                <td className="px-5 py-3 text-sm">
                  <span className="inline-flex items-center gap-1.5 px-2 py-0.5 text-[12px] font-semibold rounded-md" style={{ color: "#1D4ED8", background: "#EFF6FF" }}>
                    <Building2 size={11} strokeWidth={2.2} />{p.cities_count ?? 0}
                  </span>
                </td>
      ...
      <DeleteDialog
        ...
        province={deleteDialog.province}
