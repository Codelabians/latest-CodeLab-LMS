...
import { PORTAL } from "../routes/RouteConstants";

const BRAND = "#C90606";

/**
 * Student portal login. Unlike the admin login (which rejects the
 * `user`/student role), this posts to /user/authentication/login which
 * accepts students, then routes into the /portal layout.
 */
export default function PortalLogin() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await login({ path: "/user/authentication/login", body: form }).unwrap();
      ...
      showToast(
        err?.data?.errors?.email?.[0] || err?.data?.message || "Unable to sign in.",
        "error",
      );
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: "#FAFBFC", fontFamily: "'Montserrat', sans-serif" }}>
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-xl p-7" style={{ border: "1px solid #EEF2F6" }}>
        <div className="flex flex-col items-center mb-6">
          ...
          <h1 className="text-[18px] font-bold" style={{ color: "#0F172A" }}>Student Portal</h1>
          <p className="text-[12px]" style={{ color: "#94A3B8" }}>Sign in to view your batches, fees & attendance</p>
        </div>
