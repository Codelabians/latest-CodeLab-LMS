...
const CategoryCard = ({ category, onClick }) => {
  const getCategoryIcon = (key) => {
    ...
    return icons[key] || TrendingDown;
  };

  const getGradientColor = (key) => {
    const gradients = {
      internet: "from-blue-500 to-blue-600",
...
const AddExpenseModal = ({
  ...
  const handleSubmit = () => {
    ...
    // Include ID/uuid if editing (your backend might use `id` or `uuid`)
    if (isEditMode) {
      data.id = expense.id || expense.uuid; // supports both
      // or just: data.uuid = expense.uuid;
    }
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-16 pr-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent transition-all"
                placeholder="0.00"
                required
              />
            </div>
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button
              ...
              className="flex-1 px-6 py-3 custom-Background rounded text-white disabled:bg-gray-400 disabled:cursor-not-allowed transition-all"
            >
...
const ExpensesTable = ({ categoryId, categoryName, onBack }) => {
  ...
  const paginationMeta = expensesData?.meta?.pagination;

  useEffect(() => {
    refetch();
  }, [currentPage, itemsPerPage, refetch]);

  const mappedExpenses = expensesData?.data?.data?.map((expense) => ({
    uuid: expense.finance_uuid,
    amount: expense.amount,
    date: expense.transaction_date,
    description: expense.description,
  }));

  const [createExpense] = usePostMutation();
  const [updateExpense] = usePostMutation();
  ...
  const handleDeleteExpense = async () => {
    try {
      await deleteExpense({
        path: `/admin/finance/${selectedExpenseId}`,
        body: {},
      }).unwrap();
  ...
  const handleUpdateExpense = async (formData) => {
    try {
      await updateExpense({
        path: `/admin/finance/update/${selectedExpense.uuid}?_method=PATCH`,
        body: formData,
      }).unwrap();
    ...
    <div className="w-full">
      ...
      {!isLoading && !isError && (
        <Table
          ...
          sourceComponent="ExpenseComponent"
          setIsEditModalOpen={() => setIsEditModalOpen(true)}
          setSelectedID={setSelectedExpenseId}
...
const ExpenseCategoriesSystem = () => {
  ...
  const {
    ...
    path: "/admin/finance/categories/expense",
  });
    ...
    <div className="w-11/12 mx-auto py-8">
      <div className="mb-8">
        ...
        <p className="text-gray-600">
          Select a category to view and manage expenses
        </p>
      </div>
...
export default ExpenseCategoriesSystem;
